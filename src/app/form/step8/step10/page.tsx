...
const [thread, setThread] = useState({id: ''})
...
const handleSubmit = async () => {
    ...
    try {
        ...
        data.set('file', file)
        data.append('message', ' using the color coded system. Give a short answer.')
    ...
    setLoading(false)
        ...
        // if (response) return <p>result component and pass response to it</p>
    ...
    <> 
            <div className="w-[80%] h-full mx-auto py-[30%]  ">
                <div className="w-full   flex flex-col  place-content-center place-items-center ">
                    ...
                    <button onClick={handleSubmit} className="w-[80%] h-14 bg-purple-950 rounded-xl text-white hover:bg-purple-800 transition-colors active:bg-purple-950 flex  justify-center place-items-center absolute bottom-24" >               
                                Review
                    </button>        
                </div>
            </div>
